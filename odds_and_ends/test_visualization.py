...
class new_idea_vae_cl(nn.Module):
    def __init__(self, model_file):
        ...
        self.second_layer_parameter_size = 128  # 이전에 64였던 것을 128로 변경합니다.

        # fusion_layer3와 관련된 파라미터는 제거합니다.
        self.last_parameter_size = 128
        self.num_categories = 16

        # fusion_layer1의 입력 차원을 변경합니다.
        self.fusion_layer1 = nn.Linear(2*128*900, self.first_layer_parameter_size)  # 입력 차원을 115200으로 변경합니다.
...
model = new_idea_vae_cl('./result/Cross_vae_Linear_origin_b64_lr1e-3_4.pt').to('cuda')         #Cross_vae_Linear_origin_b64_lr1e-3_4.pt이게 뭔지 확인!
...
sns.scatterplot(x=tsne_embedded[:,0], y=tsne_embedded[:,1], hue=target_list, legend='full', palette=mymap)
plt.savefig('number1_tsne2D.png')
sns.scatterplot(x=pca_embedded[:,0], y=pca_embedded[:,1], hue=target_list, legend='full', palette=mymap)
...
for i in range (400):
    ...
    ax.scatter(tsne_df_class[i]['x'], tsne_df_class[i]['y'], tsne_df_class[i]['z'], color = random_color, label = i)
