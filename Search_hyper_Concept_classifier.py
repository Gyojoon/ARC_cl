...
def set_wandb(target_name, mode, seed, optimizer, train_batch_size, valid_batch_size, lr, dataset='ARC', entity='gyojoongu'):
    run = wandb.init(project=f'KSC_CL_{dataset}', entity=entity)
    if mode == 'train':
        config = {
            'optimizer': optimizer,
            'learning_rate': lr,
...
def objective(trial):
    ...
    if use_wandb:
        set_wandb('new_idea', 'train', seed, 'Lion', train_batch_size, valid_batch_size, lr, kind_of_dataset, entity)
